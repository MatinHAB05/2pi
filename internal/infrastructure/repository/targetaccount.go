...
func (r *targetAccountRepository) Create(ctx context.Context, target *entity.TargetAccount) error {
	...
	return db.GetDB().WithContext(ctx).Create(target).Error
}
...
func (r *targetAccountRepository) GetByID(ctx context.Context, id int64) (*entity.TargetAccount, error) {
	...
	db := database.ExtractTrxOrDB(ctx, r.db)
	err := db.GetDB().WithContext(ctx).First(&target, id).Error
...
func (r *targetAccountRepository) GetByOwnerID(ctx context.Context, ownerUserID int64, limit, offset int) ([]entity.TargetAccount, error) {
	...
	db := database.ExtractTrxOrDB(ctx, r.db)
	query := db.GetDB().WithContext(ctx).Where("owner_user_id = ?", ownerUserID)
...
func (r *targetAccountRepository) Update(ctx context.Context, target *entity.TargetAccount) error {
	db := database.ExtractTrxOrDB(ctx, r.db)
	return db.GetDB().WithContext(ctx).Where("id = ?", target.BaseEntity.ID).Updates(target).Error
}

func (r *targetAccountRepository) UpdateEnable(ctx context.Context, id int64, enb bool) error {
	db := database.ExtractTrxOrDB(ctx, r.db)
	return db.GetDB().WithContext(ctx).Model(&entity.TargetAccount{}).Where("id = ?", id).Update("enable", enb).Error
}

func (r *targetAccountRepository) Delete(ctx context.Context, id int64) error {
	db := database.ExtractTrxOrDB(ctx, r.db)
	result := db.GetDB().WithContext(ctx).Delete(&entity.TargetAccount{}, id)
	if result.Error != nil {
		return result.Error
	}
...
func (r *targetAccountRepository) GetByIDs(ctx context.Context, ids []int64) ([]entity.TargetAccount, error) {
	...
	db := database.ExtractTrxOrDB(ctx, r.db)
	if err := db.GetDB().WithContext(ctx).Where("id IN ?", ids).Find(&accounts).Error; err != nil {
		return nil, err
	}
